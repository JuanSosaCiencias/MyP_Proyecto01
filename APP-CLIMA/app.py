...
def autocompletador():
    ...
    cursor = mysql.connection.cursor()
    query = "select num_ticket from dataset where num_ticket LIKE '{}%' Limit 10".format(
        buscador)
...
def autocompletadorOrigen():
    buscador = request.form.get("text")
    cursor = mysql.connection.cursor()
    query = "SELECT DISTINCT origin FROM dataset WHERE origin LIKE '{}%' LIMIT 10".format(
        buscador)
    cursor.execute(query)
    result = cursor.fetchall()
    return jsonify(result)
...
def autocompletadorDestino():
    buscador = request.form.get("text")
    cursor = mysql.connection.cursor()
    query = "SELECT  DISTINCT destination FROM dataset WHERE destination LIKE '{}%' LIMIT 5".format(
        buscador)
    cursor.execute(query)
    result = cursor.fetchall()
    return jsonify(result)
...
def mostrar_datos():
    ...
    cursor = mysql.connection.cursor()
    if num_ticket:
        query = "SELECT * FROM dataset WHERE num_ticket = '{}'".format(
            num_ticket)
    elif ciudad_origen and ciudad_destino:
        query = "SELECT * FROM dataset WHERE origin = '{}' AND destination = '{}'".format(
